#!/usr/bin/env python
# coding: utf-8
## Tested with TF 1.14.0. Original model may user lower version than this.
## Currently, TF 1.14 comes with gast 0.5.1 which compatability issue.
## downgrading gast to lower version: gast==0.2.2 
## per https://github.com/tensorflow/tensorflow/issues/32949
...
def cudnnGraphSingleLayer(nUnits, initRNNState, batchInputs, nSteps, nBatch, nInputs, direction):
    ...
    """
    nLayers = 1
    ## TF is moving out of contrib module. Try using tf.keras.layers.CuDNNGRU instead
    ...
    def __init__(self, args):
        ...
        for t in range(30):  ## 10 days
            if 'labelsFile_'+str(t) not in self.args.keys():
                self.nDays = t
                break
        if self.args['ForTestingOnly']:
            print('running on subset of availabe data')
            self.nDays = 2  ## use 3 days of data for testing run
        print('loading data')
        ...
        #tf.random.set_seed(self.args['seed']) ## tf2.0
...
#         self.sess = tf.Session()
        ...
        if self.isTraining:
            ...
            for dayIdx in range(self.nDays):
                ...
                if self.args['synthBatchSize']>0:
                    ...
                    print('skip sythetic data for now')
                    synthIter = []
                ...
                print('making traing dataset from real data of day ', str(dayIdx))
                if realDataSize>0:
        ...
        self.rnnStartState = tf.compat.v1.get_variable('RNN_layer0/startState', [biDir, 1, self.args['nUnits']], dtype=tf.float32, initializer=tf.zeros_initializer, trainable=bool(self.args['trainableBackEnd']))
        ...
        tvars = tf.compat.v1.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES)
        ...
        print('load and initialize RNN graph')
...
#         newDataset = newDataset.apply(tf.data.experimental.shuffle_and_repeat(
#             batchSize*4, count=None, seed=None)) ## tf2
