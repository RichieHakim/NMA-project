...
def generateCharacterSequences(args):
    ...
    writer = tf.compat.v1.python_io.TFRecordWriter(args['saveFile'])
...
#                 print(charDef['charList'][classIdx])
                if penEndStates.shape == (0,0):
                    currIdx += nNewSteps
                    currentLetterIdx += 1
                    continue
                ...
                if len(charSnippets[charDef['charList'][classIdx]]) > 0:
                    choiceIdx = np.random.randint(len(charSnippets[charDef['charList'][classIdx]][0]))
                else:
                    currIdx += nNewSteps
                    currentLetterIdx += 1
            ...
            if currentSnippet.ndim < 2:
                currentSnippet = currentSnippet[None,:]
#             print(currentSnippet.shape)
            ...
            nStepsForChar = np.round(charLen*0.7 + np.random.randint(np.maximum(charLen*0.6, 1)))
